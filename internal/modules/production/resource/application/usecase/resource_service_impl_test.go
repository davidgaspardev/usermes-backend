...
import (
	...
	"github.com/davidgaspardev/usermes-backend/internal/modules/production/resource/infrastructure/adapter/output/persistence"
)
...
func TestResourceService_Create(t *testing.T) { //nolint:gocyclo
	...
	t.Run("duplicate code", func(t *testing.T) {
		...
		if err != errors.ErrCodeAlreadyExists {
...
func TestResourceService_GetByCode(t *testing.T) {
	...
	t.Run("get existing resource by code", func(t *testing.T) {
		...
		found, err := service.GetByCode(ctx, code)
		if err != nil {
			t.Fatalf("Expected no error, got %v", err)
		}
		if found == nil {
